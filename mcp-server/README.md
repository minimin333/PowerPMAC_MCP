...
## 요구 사항
- `POWERPMAC_PDK_HOME` (기본 `…\Power PMAC Manual\PDK_Reference`) — ODT DLL + 네이티브 DLL +
  라이선스 `CLLLicFile.lic`. 매니지드 PDK DLL은 csproj로 출력 폴더에 복사되고, 네이티브 런타임은
  제자리에서 사용합니다. 네이티브 검색 디렉터리는 `connect` 동안만 일시 적용합니다(전역
  `SetDllDirectory`는 C 빌드를 깨뜨림).
- `C:\DeltaTau\PowerPMAC\Compilers` (Power PMAC IDE) — ARM 크로스컴파일러(C 빌드용)와
  `rsync`/`ssh`/`sshpass`(다운로드용). 머신 환경변수 `DTBUILDPATH`로 찾습니다.
...
```
프로젝트 범위의 `..\.mcp.json`이 Claude Code에 `powerpmac` 서버로 등록합니다.
