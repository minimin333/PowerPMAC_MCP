...
2. [이 도구가 주는 것 — Skill과 MCP가 할 수 있는 일](#2-이-도구가-주는-것--skill과-mcp가-할-수-있는-일)
3. [사전 준비](#3-사전-준비)
4. [설치·등록](#4-설치등록)
5. [설치 확인](#5-설치-확인)
6. [권한 승인 이해하기](#6-권한-승인-이해하기)
7. [Skill 사용법](#7-skill-사용법--물어보기만-하면-됩니다)
8. [MCP 사용법 — 개발 워크플로](#8-mcp-사용법--개발-워크플로)
...
- **코드 리뷰·디버깅** — "왜 안 도는지"를 짚고, 흔한 함정(인덱스는 상수만, 태스크 우선순위, `save`/`reset`, 단위 혼동)을 진단합니다.
- **데이터구조·요소 조회** — `Sys.`/`Motor[]`/`Coord[]`/`Gate3[]` 등 **1,800개 이상 요소**의 의미·SAVED 여부·레거시 I-변수 대응을 설명합니다.
- **개념 설명** — 좌표계(`&`) vs 모터(`#`), 우선순위 4계층(Phase/Servo/RTI/Background), 룩어헤드, G코드, CfromScript 등.
...
- **MCP**는 **Power PMAC IDE + PDK가 설치된 Windows PC가 필수**입니다(빌드·다운로드에 컴파일러·라이선스가 필요).
...
> ⚠️ **MCP를 쓰려면 Power PMAC IDE + PDK 설치가 필수입니다.**
...
```mermaid
flowchart TD
    A["① 코드 작성·수정<br/>Skill에게 자연어로 요청"] --> B["build_project<br/>빌드 — Script + ARM C"]
    B --> C{빌드 성공?}
    C -- "아니오 · 수정" --> A
    C -- 예 --> D["download_project<br/>다운로드 → 컨트롤러 RAM"]
    D --> E["connect<br/>gpascii 세션 연결"]
    E --> F["get_response / send_command<br/>값 조회 · 명령 실행"]
    F --> G{정상 동작?}
    G -- "아니오 · 수정" --> A
    G -- 예 --> H["send_command 'save'<br/>flash에 영구 저장"]
    H --> I([완료])
```
...
| 이렇게 말하면 | Claude가 호출 |
|---|---|
| "이 프로젝트 빌드해줘" | `build_project` |
| "192.168.0.200에 다운로드해줘" | `download_project` |
| "연결하고 Motor[1].ActPos 읽어줘" | `connect` + `get_response` |
| "Motor[1].JogSpeed=10 주고 1축 +방향 조그해줘" | `send_command` (`Motor[1].JogSpeed=10`, `#1j+`) |
| "Motor[1]~Motor[4] ActPos 한 번에 읽어줘" | `get_responses` |
| "Project 폴더 목록 보여줘" | `exec_shell` (`ls -al ...`) |
| "save 해줘" | `send_command "save"` |
